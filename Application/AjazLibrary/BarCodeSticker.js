...
function GenerateSticker() {
    ...
    $.ajax
        ({
            ...
            url: "BarCode.aspx/GenerateSticker", //page/method name
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: JSON.stringify({ ItemID: itemID, ItemName: itemName, ColorID: colorID, ColorName: colorName }),
            success: LoadSticker,
        });
}
    ...
    if ($("[id$='ddlColor']").val() == "0") {
        ...
        for (var i = 0; i < dtReport.length; i++) {
            ...
            row1 += '<td style="width:12.5%;border:1px solid black;">' +
                dtReport[i].ItemName + '-' + dtReport[i].ColorName +
                '</td>';
        ...
        for (var i = 0, len = dtReport.length; i < len; i++) {
            ...
            var row1 = $(
            '<tr><td style="width:25%;border:1px solid black;">' +
            dtReport[0].ItemName + '-' + dtReport[0].ColorName +
          '</td><td style="width:25%;border:1px solid black;">' +
            dtReport[0].ItemName + '-' + dtReport[0].ColorName +
          '</td></tr>'
        );
