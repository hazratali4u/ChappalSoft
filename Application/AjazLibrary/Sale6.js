...
function LoadItems() {
    ...
    var dvLstProducts = document.getElementById("dvProducts");
    ...
    if (categoryid == 0) {
        for (var i = 0, len = lstProducts.length; i < len; ++i) {
            if ($("[id$='txtSearch']").val() != '') {
                if (lstProducts[i].Name.toLowerCase().indexOf($("[id$='txtSearch']").val().toLowerCase()) >= 0) {
                    var element = document.createElement("input");
                    element.setAttribute("type", "button");
                    element.setAttribute("value", lstProducts[i].Name);
                    element.setAttribute("title", lstProducts[i].Name);
                    element.setAttribute("name", lstProducts[i].Name);
                    element.setAttribute("id", lstProducts[i].ItemID);
                    element.setAttribute("class", "btn btn-inverse-info btn-lg");
                    element.setAttribute("style", "margin:5px;width:95%;height:85px;padding:0px;");
                    element.onclick = function () {
                        ItemButtonClick(this.id);
                    };
        ...
        for (var i = 0, len = lstProducts.length; i < len; ++i) {
            if (lstProducts[i].CategoryID == categoryid) {
                if ($("[id$='txtSearch']").val() != '') {
                    if (lstProducts[i].Name.toLowerCase().indexOf($("[id$='txtSearch']").val().toLowerCase()) >= 0) {
                        var element = document.createElement("input");
                        element.setAttribute("type", "button");
...
function LoadColor() {
    var dvLstColors = document.getElementById("dvColors");
    while (dvLstColors.hasChildNodes()) {
        dvLstColors.removeChild(dvLstColors.lastChild);
    }
    var lstColors = $("[id$='hfColorIDs']").val();
    lstColors = eval(lstColors);
    for (var i = 0, len = lstColors.length; i < len; ++i) {
        if ($("[id$='txtSearchColor']").val() != '') {
            if (lstColors[i].Name.toLowerCase().indexOf($("[id$='txtSearchColor']").val().toLowerCase()) >= 0) {
                var element = document.createElement("input");
                element.setAttribute("type", "button")
                if (lstColors[i].ShowName) {
                    element.setAttribute("value", lstColors[i].Name);
                    element.setAttribute("name", lstColors[i].Name);
                    element.setAttribute("style", "margin:5px;width:95%;height:50px;padding:0px;border-color:#198ae3;");
                }
                else {
                    element.setAttribute("value", "");
                    element.setAttribute("name", "");
                    element.setAttribute("style", "margin:5px;width:95%;height:50px;padding:0px;border-color:#198ae3;background-color:" + lstColors[i].ColorCode + ";");                    
                }
                element.setAttribute("title", lstColors[i].Name);
                element.setAttribute("id", lstColors[i].ColorID);
                element.setAttribute("class", "btn btn-inverse-info btn-lg");                
                element.onclick = function () {
                    ColorButtonClick(this.id);
                };
                dvLstColors.appendChild(element);
            }
        }
        else {
            var element = document.createElement("input");
