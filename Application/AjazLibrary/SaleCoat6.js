...
function AddItemToGrid() {
    ...
    $('#tblItemGrid tr').each(function (row, tr) {
        if ($(tr).find("td:eq(0)").text() == itemid && $(tr).find("td:eq(6)").text() == colorid) {
            flag = true;
            return;
        }
    });
...
function qtyBlur(obj) {
    ...
    if (!/^\d+$/.test($(rowIndex).find('td:eq(4) input').val()) || $(rowIndex).find('td:eq(4) input').val() === "") {
        $(rowIndex).find('td:eq(5) input').val(0);
        return;
    }
    ...
    $(rowIndex).find('td:eq(5) input').val(amount);
...
function SetTotal() {
    ...
    $('#tblItemGrid tr').each(function (row, tr) {
        if ($(tr).find("td:eq(5) input").val() != "") {
            amount += parseInt($(tr).find("td:eq(5) input").val());
            qty += parseInt($(tr).find("td:eq(4) input").val());
        }
    });
...
function SavePayment() {
    ...
    $('#tblItemGrid tr').each(function (row, tr) {
        if ($(tr).find("td:eq(5) input").val() != "") {
            grossamount += parseInt($(tr).find("td:eq(5) input").val());
        }
    });
    var discount = 0;
    var amountpaid = 0;
...
function btnDoneSizeClicked() {
    ...
    $('#tblItemGrid tr').each(function () {
        if (parseInt($(this).find("td:eq(4) input").val()) > 0) {
...
function CheckStock() {
    ...
    $('#tblItemGrid tr').each(function () {
        ...
        var colorID = $(this).find("td:eq(6)").text().trim();
